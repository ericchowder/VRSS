...
public class rightHandThumb : MonoBehaviour
{
	...
	float RIX = 0, RIY = 0, RIZ = 0, RTX = 0, RTY = 0, RTZ = 0, RHX = 0, RHY = 0, RHZ = 0, LIX = 0, LIY = 0, LIZ = 0, LTX = 0, LTY = 0, LTZ = 0, LHX = 0, LHY = 0, LHZ = 0;
	void Start()
	{
		sp.Open();
		sp.ReadTimeout = 1;
		...
		*/
		...
		if (sp.IsOpen)
		{
			try
			{
				...
				// Accel: X = L/R, Y = F/B, Z = U/D (Y & Z are backwards)
				// uX = aX, uZ = -aY, uY = -aZ
				// --- GYROSCOPE IS ALSO FLIPPED ---
		...
		transform.Rotate(RTX*0.01f, RTY*0.01f, RTZ*0.01f);
	}
